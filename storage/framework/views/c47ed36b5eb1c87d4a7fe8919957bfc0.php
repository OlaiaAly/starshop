...
<script type="text/javascript">
    $.ajaxSetup({
        ...
        $.ajax({
            type: 'GET',
            url: '/product/view/model/' + id, // Ajuste a rota conforme necessário
            dataType: 'json',
            success: function(data) {
                ...
                $('#pprice').text(data.product.selling_price + ' MZN');
                ...
                $('#pimage').html('<img src="' + data.product.image + '" alt="' + data.product.product_name + '" style="max-width: 100%;">');

                // Preço com e sem desconto
                if (data.product.discount_price == null) {
                    $('#ppprice').text(data.product.selling_price);
                    $('#oldprice').text('');
                } else {
                    $('#ppprice').text(data.product.discount_price);
                    $('#oldprice').text(data.product.selling_price);
                }

                // Preencher opções de tamanhos e cores
                populateOptions('size', data.size);
                populateOptions('color', data.color);

                // Exibição de estoque
                if (data.product.product_qty > 0) {
                    $('#aviable').show().text('Disponível');
                    $('#stockout').hide();
                } else {
                    $('#aviable').hide();
                    $('#stockout').show().text('Fora de Estoque');
                }
...
// Função auxiliar para preencher opções de select
...
</html><?php /**PATH C:\Users\Cuata\Desktop\laravel10\resources\views/frontend/master_dashboard.blade.php ENDPATH**/ ?>